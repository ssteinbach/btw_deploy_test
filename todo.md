...
Small stuff:
===========
- add no-bomb stun animation for bombgoblin
- more telegraphing the bombgoblin throw (flash red before throw)

Big Stuff
=========
- proper intro for player (maybe with special juice?)
- drop shadows
- add better death -> you failed transition (really unclear)
- add new enemy type (try and add more variety for longer playtime)
    - maybe a ghost that charges down a row or column
- sword swing knocks back bombs
- bomb explosions push back enemies
- spelling mistakes on title screen don't read as a joke
- Telegraph controls
- retune spawn time (esp. between laser and falling anim)
    - longer dead time between waves, but less dead time at the beginninig
- real deal title sequence
- dust with blue glow for summoning anim
...
- resume instead of restart music when going resetting after dying
- make every button swing if swing is the only verb 
- fix bug where a bombgoblin finishes throw if you hit them mid throw
- add ouch sound
- adjust spawn laser z depth to be in front of player if below player
- player should 'fall' into the arena just before the first wave (reduce dead time)
- lock controls after taking damage?
- similar bug with getting hit mid swing (interruptable animation system)?
